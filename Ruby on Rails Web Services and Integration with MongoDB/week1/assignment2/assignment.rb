...
class Solution
  ...
  def load_collection(file_path) 
    x = self.class.load_hash(file_path)
    @coll.insert_many(x)
  end
  ...
  def find_by_name(fname, lname)
    prototype = {:first_name => fname, :last_name => lname}
    @coll.find(prototype).projection(first_name:true, last_name:true, number:true, _id:false)
  end
